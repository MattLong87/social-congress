...
function displayLegislators(state){
	...
	state.legislators.map(function(legislator){
		...
		var chamber = (legislator.urls && legislator.urls[0].includes("senate")) ? "Senator" : "Representative";
		...
		$(".results").append(
			"<div class = 'twitter-embed'><span class = 'party " + legislator.party + "'>" + party + "</span><span class = 'name'>" + legislator.name + "</span>" + html1 + twitterHandle + html2 + twitterHandle + html3 + tweetButton + "</div>"
			);
	});
};
